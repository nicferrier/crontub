h1. crontub - a dynamic cron daemon

p. crontub is a virtual crontab... it abstracts the definition of the
...
p. Why? crontub let's your programmers specify how and where a script
...
p. In addition, crontub let's you specify a role checker so that you can
...
p. 'pidfile' is a path to a pidfile, where the daemon records it's pid,
by default the pidfile is in /tmp/crontub.pid

p. 'roletest' specifies that somecommand be run to establish whether the
cron should be run here or not.

p. the roletest command is executed with the %% template variables:

bq. 'role' set to the role being tested
'user' set to the user being tested

p. for example, setting 'roletest' thusly:

pre. roletest = /usr/local/bin/check_is_staging -u %(user)s %(role)s

p. with a role of "staging" and a user of "woome" would cause the
